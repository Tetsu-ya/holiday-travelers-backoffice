...
@section('title', 'Roles & Permissions')
@section('content')
<div class="space-y-6"><div><h2 class="font-heading text-xl font-semibold text-primary">Roles & permissions</h2><p class="text-sm text-gray-500">Configure the access matrix used by your staff roles.</p></div><div class="overflow-x-auto rounded-2xl border border-border bg-card shadow-sm"><form method="POST" action="{{ route('roles-permissions.update') }}">@csrf<div class="border-b border-border p-5"><label class="mb-2 block text-sm font-medium">Role to update</label><select name="role" class="rounded-lg border border-border bg-card px-3 py-2">@foreach($roles as $role)<option value="{{ $role }}">{{ ucfirst($role) }}</option>@endforeach</select></div><table class="w-full text-left text-sm"><thead class="text-gray-500"><tr><th class="px-5 py-3">Permission</th>@foreach($roles as $role)<th class="px-5 py-3 capitalize">{{ $role }}</th>@endforeach</tr></thead><tbody class="divide-y divide-border">@foreach($permissions as $key => $permission)<tr><td class="px-5 py-3 font-medium text-primary">{{ $permission }}</td>@foreach($roles as $role)<td class="px-5 py-3"><span class="{{ in_array($key, $rolePermissions[$role]) ? 'text-success' : 'text-gray-400' }}">{{ in_array($key, $rolePermissions[$role]) ? 'Enabled' : 'Restricted' }}</span></td>@endforeach</tr>@endforeach</tbody></table><div class="border-t border-border p-5"><p class="mb-3 text-xs text-gray-500">To customize a role, select it above and submit the permissions below.</p><div class="grid gap-3 sm:grid-cols-2">@foreach($permissions as $key => $permission)<label class="flex items-center gap-2 text-sm"><input type="checkbox" name="permissions[{{ $key }}]" value="1" @checked(in_array($key, $rolePermissions['staff']))><span>{{ $permission }}</span></label>@endforeach</div><button class="mt-5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white">Save permissions</button></div></form></div></div>
@endsection
